...
- 实时终端隐藏内部回显行时必须保留该行的换行语义，避免下一次 shell prompt 被写到上一条 prompt 同一行；marker 结果行仍完全隐藏。
...
- 2026-06-30：修复终端隐藏 Agent 内部回显行后 prompt 换行丢失的问题，保留内部回显行换行语义并补充工具测试；涉及 `App.tsx`、`agentCommand.ts`、`utils.ts`。验证命令：`npm run test:desktop:agent-command`、`npm run typecheck:desktop`。
