...
- 当前已开始 Milestone 1，已实现主机列表、会话接口、SSE 输出流、input 写入接口、`local-demo` 终端演示链路、临时密码认证 SSH 连接、工作台主布局、服务器新增/编辑/删除/导入导出、服务器基础配置本地 JSON 持久化，以及密码 / SSH Key 系统安全存储
...
12. 完成服务器新增、编辑、删除、导入导出
13. 完成服务器基础配置本地 JSON 持久化
14. 完成系统安全存储保存密码和 SSH Key
15. 接入私钥 / agent 认证与 known_hosts 校验
