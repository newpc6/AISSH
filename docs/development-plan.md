...
- 当前已开始 Milestone 1，已实现主机列表、会话接口、SSE 输出流、input 写入接口、`local-demo` 终端演示链路、临时密码认证 SSH 连接、工作台主布局，以及内存态服务器新增/编辑/删除/导入导出
...
12. 完成内存态服务器新增、编辑、删除、导入导出
13. 接入本地持久化与系统安全存储
14. 接入私钥 / agent 认证与 known_hosts 校验
