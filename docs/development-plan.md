...
- 当前已开始 Milestone 1，已实现主机列表、会话接口、SSE 输出流、input 写入接口、`local-demo` 终端演示链路、临时密码认证 SSH 连接、工作台主布局、左侧新增 SSH 连接弹窗、SSH Key 文件选择、保存失败弹窗内反馈、保存成功自动选中新服务器、服务器新增/导入导出、服务器基础配置本地 JSON 持久化，以及密码 / SSH Key 系统安全存储
...
16. 接入私钥 / agent 认证与 known_hosts 校验
