...
- 顶部下拉菜单、设置弹窗、会话 tab 关闭确认
- 真实命令历史记录，已过滤路径跟踪内部探测命令，只记录用户回车提交的完整命令
- 会话 tab 连接状态圆点、关闭后自动切换剩余会话、无会话空状态和最近服务器快捷连接
- SFTP 文件浏览、目录进入、上传、下载和基础传输进度
- 当前服务器 CPU、内存、硬盘、网络指标轮询展示
- AI 大模型地址、Key、模型设置入口和命令预测交互占位
...
10. 点击服务器卡片右上角 `⋯` 可打开菜单，执行编辑、连接、复制配置和删除

说明：

- 当前服务器基础配置会保存在用户配置目录下的 `ai-ssh/hosts.json`。Windows 通常是 `%APPDATA%\ai-ssh\hosts.json`
...
2. 双击文件夹进入目录
3. 右键或双击文件可以下载
4. 点击 `上传` 或把本地文件拖进文件面板可以上传，支持多个文件
5. 面板底部会展示最近上传/下载任务的基础进度
6. 勾选 `跟踪终端路径` 后，系统会被动识别常见 shell prompt 中的当前目录并尝试刷新文件面板路径

说明：

- SFTP 文件能力需要真实 SSH 主机支持 SFTP 子系统
- 路径跟踪不再向远端自动注入 `printf` 命令，因此不会污染终端历史；如果远端自定义 prompt 不包含 `user@host:/path$` 这类路径信息，文件面板可能需要手动刷新或手动进入目录
...
- 设置服务器信息刷新频率，默认 2 秒
- 设置 AI 大模型地址、API Key 和模型名。AI 地址按 OpenAI 兼容接口的 base URL 填写，例如 `https://api.openai.com/v1` 或本地兼容服务 `http://127.0.0.1:11434/v1`
- 开启或关闭 AI 命令预测

说明：

- 当前 AI 预测已经具备交互占位、历史命令上下文和 Tab 应用逻辑
- 地址填写 base URL，不需要填写到 `/chat/completions`；后续真实 provider 会在 base URL 后拼接对应接口路径
- 当前设置仍是前端运行期状态，后续会持久化到本地设置存储
- 真正调用大模型生成预测结果会在后续接入 AI provider 后完成
