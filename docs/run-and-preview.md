...
8. 点击顶部 `导出` 可以把当前服务器列表复制到剪贴板
9. 点击顶部 `导入` 会导入一条示例服务器

说明：

- 当前服务器基础配置会保存在用户配置目录下的 `ai-ssh/hosts.json`
- 如果需要临时指定存储位置，可以在启动 Go core 前设置 `AI_SSH_HOSTS_PATH`
- 密码和 SSH Key 会写入操作系统安全存储，不会写入本地 JSON，也不会出现在导出结果中
- SSH Key 会分片写入系统安全存储，以兼容较长私钥内容
- 如果系统安全存储不可用或保存失败，新增对话框会显示具体错误
- 当前为了先打通连接链路，host key 使用临时宽松校验；后续会补 known_hosts 校验和 agent 认证

### 查看运行日志

当前可以通过顶部菜单中的 `工具` 打开运行日志面板：

1. 点击顶部菜单栏 `工具`
2. 在弹窗中查看最近的前端请求、Go core 请求和错误信息
3. 通过右上角日志级别下拉框切换 `debug`、`info`、`warn`、`error`
4. 点击 `刷新` 可以重新从 Go core 拉取最新日志
