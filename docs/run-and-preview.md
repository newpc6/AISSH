...
- SFTP 文件浏览、路径输入进入、目录进入、上传、下载和基础传输进度
- 当前服务器 CPU 当前使用率、内存折线趋势、磁盘列表、网络实时上下行带宽展示
...
8. 在顶部 `文件` 菜单中可以选择导出服务器列表、导出软件配置、导入服务器列表或导入软件配置
...
3. 双击文件夹进入目录
4. 右键或双击文件可以下载
5. 点击 `上传` 或把本地文件拖进文件面板可以上传，支持多个文件
6. 面板底部会展示最近上传/下载任务的基础进度
7. 勾选 `跟踪终端路径` 后，系统会被动识别常见 shell prompt 中的当前目录并尝试刷新文件面板路径
