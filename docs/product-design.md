...
- 顶部菜单栏：文件、编辑、会话、传输、工具、设置均采用下拉菜单；文件菜单提供服务器列表导入导出、服务器列表含加密凭据导出、软件配置导入导出等分类型入口；工具菜单提供运行日志入口，用于排查连接、保存、接口调用和核心服务问题；设置菜单提供刷新频率、指标趋势时间范围和 AI provider 配置入口
...
- 中间主工作区：SSH 终端会话占据最大面积，顶部提供会话 tabs，用于快速切换不同服务器；每个 tab 提供无边框关闭按钮和状态圆点，关闭前需要确认，关闭当前 tab 后自动切换剩余会话；没有打开会话时不显示伪 tab 和连接状态，只展示空状态和最近服务器快捷连接
- 右侧信息与工具栏：上方显示当前 tab 对应 SSH 服务器信息；CPU、内存、磁盘、网络通过 Go core 读取远端 `/proc` 与 `df` 原始数据解析，展示当前百分比、可配置时间范围折线趋势、多个挂载点列表、实时下载/上传带宽与累计流量；下方提供 AI 工具、历史命令、快捷命令和预测建议，不承载连接录入表单
...
- 连接流程足够快
- 主机 / 会话状态清晰
- 会话输出流失败时不能长期停留在 connecting，必须切换到错误状态并提示用户查看日志
