...
- 顶部菜单栏：文件、编辑、会话、传输、工具、设置，以及导入、导出等入口；工具菜单提供运行日志入口，用于排查连接、保存、接口调用和核心服务问题
...
- 左侧模式切换：SSH 主机列表与远程文件目录两种模式，文件模式参考 MobaXterm 的远程文件浏览逻辑
- 中间主工作区：SSH 终端会话占据最大面积，顶部提供会话 tabs，用于快速切换不同服务器
- 右侧信息与工具栏：上方显示当前 SSH 服务器信息，下方提供 AI 工具、历史命令、快捷命令和预测建议，不承载连接录入表单
- AI 预测区域：启用后读取终端上下文，预测下一步命令，后续支持 Tab 应用建议
