...
- 顶部菜单栏：文件、编辑、会话、传输、工具、设置均采用下拉菜单；文件菜单提供服务器列表导入导出、软件配置导入导出等分类型入口；工具菜单提供运行日志入口，用于排查连接、保存、接口调用和核心服务问题；设置菜单提供刷新频率、指标趋势时间范围和 AI provider 配置入口
...
- 密码与 SSH Key 通过操作系统安全存储保存，并在连接前由 Go core 读取使用；较长 SSH Key 需要分片写入以兼容系统凭据长度限制
- Windows 当前默认路径通常为 `%APPDATA%\ai-ssh\hosts.json`，也可以通过 `AI_SSH_HOSTS_PATH` 覆盖
- SQLite：存业务元数据
