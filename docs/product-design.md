...
- 顶部菜单栏：文件、编辑、会话、传输、工具、设置均采用下拉菜单；工具菜单提供运行日志入口，用于排查连接、保存、接口调用和核心服务问题；设置菜单提供刷新频率和 AI provider 配置入口
- 左侧资源栏：SSH 服务器列表、分组、标签、导入导出入口；点击加号弹出新增 SSH 连接对话框；每个服务器卡片右上角提供更多菜单，先支持编辑、连接、复制配置和删除
- 新增连接对话框：填写地址、端口、用户、认证方式，并可勾选保存密码或 SSH Key；SSH Key 支持粘贴或选择本地私钥文件，保存后连接时自动使用；保存成功后自动关闭并选中新服务器，保存失败在对话框内显示具体错误
- 左侧模式切换：SSH 主机列表与远程文件目录两种模式，文件模式使用 SFTP 展示当前路径的文件和目录，支持名称、大小、修改日期、双击进入目录、上传、下载和基础传输进度
- 中间主工作区：SSH 终端会话占据最大面积，顶部提供会话 tabs，用于快速切换不同服务器；每个 tab 提供无边框关闭按钮和状态圆点，关闭前需要确认，关闭当前 tab 后自动切换剩余会话，全部关闭后展示空状态和最近服务器快捷连接
- 右侧信息与工具栏：上方显示当前 tab 对应 SSH 服务器信息和 CPU、内存、硬盘、网络指标，下方提供 AI 工具、历史命令、快捷命令和预测建议，不承载连接录入表单
