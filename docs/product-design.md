...
- 左侧模式切换：SSH 主机列表与远程文件目录两种模式，文件模式使用 SFTP 展示当前路径的文件和目录，支持路径输入回车进入、名称、大小、修改日期、双击进入目录、上传、下载和基础传输进度
- 中间主工作区：SSH 终端会话占据最大面积，顶部提供会话 tabs，用于快速切换不同服务器；每个 tab 提供无边框关闭按钮和状态圆点，关闭前需要确认，关闭当前 tab 后自动切换剩余会话，全部关闭后展示空状态和最近服务器快捷连接
- 右侧信息与工具栏：上方显示当前 tab 对应 SSH 服务器信息；CPU 使用短间隔采样得到当前使用率，CPU 和内存展示当前百分比与可配置时间范围折线趋势，硬盘展示多个挂载点列表，网络展示实时下载/上传带宽与累计流量；下方提供 AI 工具、历史命令、快捷命令和预测建议，不承载连接录入表单
...
- 终端命令历史必须以用户回车提交的完整命令为准，内部路径跟踪或系统探测命令不能污染历史
