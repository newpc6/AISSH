...
- 左侧资源栏：SSH 服务器列表、分组、标签；点击加号弹出新增 SSH 连接对话框；每个服务器卡片右上角提供更多菜单，先支持编辑、连接、复制配置和删除；左侧宽度默认保证文件名可读，并支持拖动调整
- 新增连接对话框：填写地址、端口、用户、认证方式，并可勾选保存密码或 SSH Key；SSH Key 支持粘贴或选择本地私钥文件，保存后连接时自动使用；保存成功后自动关闭并选中新服务器，保存失败在对话框内显示具体错误
